...
SELECT ename FROM EMPLOYEE WHERE MOD(MANAGER, 2) = 1;
...
SELECT TO_CHAR(hiredate, 'YY/MM DY') FROM EMPLOYEE;
...
SELECT TRUNC(SYSDATE - TO_DATE(19931113, 'YYYYMMDD')) FROM dual;

-- 5-2. 자신이 태어난 날짜에서 현재까지 몇 개월이 지났는지 출력 하세요.
SELECT TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(19931113, 'YYYYMMDD'))) FROM dual; 

-- 6. 사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 null 갑대신 0으로 출력 하시오.
SELECT NVL2(manager, manager, 0) FROM EMPLOYEE;

-- 7. DECODE 함수로 직급에 따라 급여를 인상하도록 하시오. 직급이 'ANAIYST' 사원은 200 , 'SALESMAN' 사원은 180, 'MANAGER'인 사원은 150, 'CLERK'인 사원은 100을 인상하시오.
SELECT ename, DECODE(job, 'ANALYST', salary + 200,
	'SALESMAN', salary + 180,
	'MANAGER', salary + 150,
	'CLERK', salary + 100,
	SALARY) AS "인상된 급여"
